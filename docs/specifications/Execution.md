...
requirements plus all revisions made during design review (server-side timing,
authenticated Student exam access, autosave/resume, duplicate-submission prevention, default Admin provisioning,
...
| **Student** | A registered user with an `@student.mau.edu.ng` address and a persistent dashboard. A Student may accept invitations to multiple courses; examinations, attempts, and results are grouped by course. Exam access requires an authenticated Student session and accepted enrollment. |
...
| FR2 | Role-Based Access Model | Admin, Lecturer, and Student accounts use one password-protected login page with server-side role checks and role-based dashboard redirection. The Admin remains pre-provisioned and has no registration route. | Flask-Login, SQLAlchemy |
...
| FR6 | Student Examination Access | Only a logged-in Student with accepted enrollment in the examination's course may open, resume, submit, or read that examination. Opening it from the Student dashboard starts or resumes the single attempt directly without a second email or OTP prompt. | Flask-Login, role decorators, SQLAlchemy enrollment checks |
...
| FR17 | Autosave & Resume | The system shall periodically persist a Student's in-progress responses and allow the logged-in Student to resume an interrupted attempt from the dashboard without a separate exam credential. | Vanilla JS (`fetch`, debounced interval), Flask endpoint, SQLAlchemy (JSON `responses` column), authenticated account session |
...
| NFR2 | Access Control | Admin access is limited to the pre-provisioned account; Lecturer access requires exact-domain email verification plus Admin approval; Student registration requires the exact `student.mau.edu.ng` domain; every examination route additionally requires a logged-in Student with accepted course enrollment. | Flask-Login, role decorators, exact-domain validation, SQLAlchemy enrollment checks |
...
### 4.2 Authenticated Student Examination Access
- All roles use one login form; successful credentials determine the dashboard destination.
- The Student opens an available examination from the course section of the Student dashboard.
- The start action requires a valid Student login and accepted enrollment in the
  examination's course; possession of a forwarded exam link alone is insufficient.
- The server creates or resumes the Student's one attempt and redirects directly to
  the examination session. It does not ask for the Student's name, email, OTP, or
  magic-link verification again.
- Every question, timer, autosave, supervision, evidence, submission, receipt, and
  result endpoint rechecks the authenticated account and course enrollment.
...
- On authenticated entry, the server records `started_at` and becomes the sole authority on remaining time; the client-side countdown is a display only, resynced periodically against the server (FR18).
...
- If the Student's browser or connection drops mid-exam, signing back into the account and reopening the examination resumes the same attempt — no separate exam verification is required.
